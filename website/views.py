...
# Import the updated similarity service
from .similarity_service import FineTunedSimilarityService

# Initialize the fine-tuned similarity service
...
def submit_detailed_lost():
    ...
    try:
        ...
        # NEW: Check for similar items in found items database using fine-tuned models
        print("=== STARTING SIMILARITY CHECK WITH FINE-TUNED MODELS ===")
        try:
            ...
            # Find similar items with fine-tuned models
            ...
            if similar_items:
                print(f"Found {len(similar_items)} similar items using fine-tuned models")
                for item, score in similar_items[:3]:
...
def suggestions(lost_item_id):
    ...
    try:
        ...
        # Filter items with similarity > 0.1
...
def submit_found():
    """Redirect to found item form"""
    return redirect(url_for('views.found_item_form'))

@views.route('/submit-initial-found', methods=['POST'])
def submit_initial_found():
    """Handle initial found item submission"""
    print("=== DEBUG: Initial found form submission received ===")
    print(f"Form data: {dict(request.form)}")
    ...
    try:
        ...
        # Insert into database
        result = db.foundItems.insert_one(initial_data)
        item_id = str(result.inserted_id)
        
        print(f"Initial found item data inserted with ID: {item_id}")
        
        # Redirect to detailed form with the item ID
        return redirect(url_for('views.found_item_form', item_id=item_id))
        
    except Exception as e:
        print(f"Error submitting initial found item: {e}")
        flash('An error occurred. Please try again.', 'error')
        return redirect(url_for('views.welcome'))
